...
function findFirstSingleChar(str) {
  const arr = [...str];
  return arr[arr.findIndex((char) => arr.indexOf(char) === arr.lastIndexOf(char))] || null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${leftBracket}${nums}${rightBracket}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openBrackets.includes(cur)) {
      ...
      if (brackets[cur] === buffer[buffer.length - 1]) {
        buffer.pop();
      } else return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  PathesArr[0].forEach((folder, index) => {
    if (PathesArr.every((el) => el[index] === folder)) {
      res += `${folder}/`;
    }
  });
  return res;
...
function getMatrixProduct(m1, m2) {
  const pm = new Array(m1.length);
  for (let i = 0; i < m1.length; i += 1) {
    pm[i] = new Array(m2[0].length);
